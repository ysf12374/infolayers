...
@action('cluster/broadband.topojson', method=['GET', 'POST'])
@action.uses(CORS(), LocalsOnly())
@action.uses('../planetclient/templates/generic.topo.json')
def generic_fetchtiled_topo():
    return dict(topology=brap(source_name='AGCOM')(fetchtiled_)())

@action('broadband.topojson', method=['GET', 'POST'])
@action.uses(CORS(), LocalsOnly())
@action.uses('../planetclient/templates/generic.topo.json')
def generic_fetch():
    return dict(topology=brap(source_name='AGCOM')(fetch_)())
